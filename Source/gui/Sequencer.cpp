...
void Sequencer::buttonClicked( Button* b )
{
	...
	else if ( b == previous || b == next )
	{
		...
		if ( b == previous )
			chaseManager->skipToPreviousSequence();


		else if ( b == next )
			chaseManager->skipToNextSequence();
	...
	{
		if ( b == lessSteps )
			chaseManager->removeStep();

		else if ( b == moreSteps )
			chaseManager->addStep();
...
void Sequencer::labelTextChanged( Label* labelThatHasChanged )
{
	...
	else if ( labelThatHasChanged == stepNumber )
	{
		...
		if ( desiredStepCount > 0 )
			chaseManager->setStepCount( desiredStepCount );
		updateSequenceSettings();
	}
}
...
{
	chaseManager->skipToNextStep();
	updateSequenceSettings();
}

void Sequencer::previousStep()
{
	chaseManager->skipToPreviousStep();
	updateSequenceSettings();
}

void Sequencer::selectStep( int i )
{
	chaseManager->skipToStep( i );
	updateSequenceSettings();
}
...
void Sequencer::updateSequenceSettings()
{
	...
	stepNumber->setText( String( chaseManager->getLastStepIndex() + 1 ), dontSendNotification );

	//retrigger the current step
	stepper->triggerButton( chaseManager->getCurrentStepIndex() );
	...
	float propX = float( chaseManager->getCurrentStepIndex() ) / float( chaseManager->getLastStepIndex() );
